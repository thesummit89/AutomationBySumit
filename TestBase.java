...
public class TestBase {
    ...
    public static Properties extentProp = new Properties();
    ...
    public void reportPathSetup() {
        ...
        try {
            ...
            properies.load(fis);
            createFolderPath();
            FileInputStream inputReader = new FileInputStream(Constants.SPARK_CONFIG);
            extentProp.load(inputReader);
    ...
    private static void saveProperties(Properties p) throws IOException, IOException {
        FileOutputStream fr = new FileOutputStream(Constants.SPARK_CONFIG);
        p.store(fr, "Updated Extent Report Properties");
        fr.close();
    ...
    public void instance(ITestContext context) throws IOException {
        ...
        extentProp.setProperty("extent.reporter.spark.out",executionReportPath);
        extentProp.setProperty("screenshot.dir",Constants.SCREENSHOT_PATH_FAIL);
        saveProperties(extentProp);
    ...
    public static void takeScreenshot(Scenario scenario) {
        ...
        try {
            if (scenario.isFailed()) {
                ...
                scenario.attach(encodedBase64,"image/png", "Failed_Screenshot");
                extentProp.setProperty("screenshot.rel.path",screenshotPath);
                saveProperties(extentProp);
            }
        } catch (Exception e) {
   ...
   // @AfterMethod(alwaysRun = true)
    public static void endTestCase(Scenario scenario) {
        ...
        log4jInfo("********Execution completed for_" + scenario.getName() + "_ *****");
    }
